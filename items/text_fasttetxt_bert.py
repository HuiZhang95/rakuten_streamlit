...
def text_fasttetxt_bert():
	...
	st.write( 'Overall the classification with the fastText library offers the possibility for multi-class '
			'text classification. It is relatively easy to use and the adaptation to new datasets can be '
			'performed on a short time scale. Compared to more complex large NLPs the performance in terms '
			'of the weighted F1-score is not as good. The F1-score above 0.80 is still an acceptable '
	...
	with st.expander('KPI of the fastText model: Classification report and confusion matrix'):
	...
	st.write( 'In natural language processing NLP tasks, transformer-based models like BERT '
			...
			'corpus of text input. This pre-trained models guide as a starting point for the training '
			'of custom datasets. The pre-trained model is then introduced to the custom dataset '
			'by further training the model in order to adapt to the features of the custom dataset. '
	...
	with st.expander('Training curve of a roBERTa-large model'):	
		img = Image.open( work_dir + image_dir + 'image_04.png' )
		st.image(img, use_container_width = True)
		
	st.write( 'The final KPIs are depicted in image below. The class report shows that compared '
	...
	with st.expander('KPI of the roBERTa-large model: Classification report and confusion matrix'):	
	...
	with st.expander('Visualization of the text classification with PCA and t-SNE'):	
